...
class Authorization
{
    ...
    public function login()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST'):
            if(isset($_POST['email']) && isset($_POST['password']) && isset($_POST['csrf_token']) && auth::validateToken($_POST['csrf_token'])):
                ...
                $hourAgo = time() - 60 * 60;
                $data = query()->select('SELECT email,users.id,password,roll,verified,COUNT(loginattempts.id) FROM users LEFT JOIN loginattempts ON users.id = user AND timestamp > :timestamp WHERE email =:email GROUP BY users.id', array(
                    ':timestamp' => $hourAgo,
                    ':email' => $email
                ),true);
                // print_r($data);
                    ...
                    if ($data->data[0]->verified):
                        ...
                        if ($data->data[0]->$count <= MAX_LOGIN_ATTEMPTS_PER_HOUR):
                            ...
                            if (password_verify($password, $data->data[0]->password)):
                                $_SESSION['IntrbologinEmail'] = $data->data[0]->email;
                                $_SESSION['roll'] = $data->data[0]->roll;
    ...
    public function registeration()
    {
        ...
        else if (!checkdnsrr(substr($_POST['email'], strpos($_POST['email'], '@') + 1) , 'MX') && !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL))
        {
            $errors[] = 3;
        }
        ...
        if (count($errors) === 0)
        {
            if (isset($_POST['csrf_token']) && auth::validateToken($_POST['csrf_token']))
            {
                    ...
                    if ($data && $data->count === 0)
                    {
                        ...
                        $sendTo = array('email'=>$_POST['email'],'name' =>$name,'subject'=>'Welcome "brand name" ','body' => $body);
          
                        $id = (int)query()->insert('INSERT INTO users (name,lastName,email,password,roll) VALUES (?,?,?,?,?)',array($name,$lastName,$_POST['email'],$hash,'user'));
    ...
    public function updateAcount()
    {
        ...
        if (isset($_POST['id']) && isset($_SESSION['IntrbologinEmail']) && isset($_POST['csrf_token']) && auth::validateToken($_POST['csrf_token'])):
        ...
        if (isset($_POST['OTP']) && isset($_SESSION['otpEmail']) && isset($_POST['csrf_token']) && auth::validateToken($_POST['csrf_token'])):
             ...
             if($data->data[0]->otp == $_POST['OTP']):
               ...
               if(query()->update("UPDATE users SET verified=:num WHERE email=:email", array(':num'=> 1,':email' => $_SESSION['otpEmail']))):
                ...
                $_SESSION['IntrbologinEmail'] = $_SESSION['otpEmail'];
